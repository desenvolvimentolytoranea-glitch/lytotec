...
-- Recriar a função com SECURITY DEFINER para dar as permissões necessárias
CREATE OR REPLACE FUNCTION atualizar_status_equipamentos_nao_apontados()
RETURNS INTEGER 
LANGUAGE plpgsql
SECURITY DEFINER -- Adiciona privilégios elevados para executar a função
AS $$
DECLARE
...
-- Atualizar o comentário da função
COMMENT ON FUNCTION atualizar_status_equipamentos_nao_apontados() IS 
'Atualiza para Disponível todos os equipamentos que não foram apontados hoje, exceto os que estão Em Manutenção. Executa com privilégios elevados.';
