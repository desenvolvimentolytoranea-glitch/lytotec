...
export default function Dashboard1() {
  console.log("Dashboard1 component rendering");
  const navigate = useNavigate();
  const [period, setPeriod] = useState<PeriodFilter>("day");
  const [isLoaded, setIsLoaded] = useState(false);
  
  // Use the permission guard with the dashboard1 route
  const { isLoading: authLoading } = usePermissionGuard({
  ...
  useEffect(() => {
    ...
    const initialLoadTimer = setTimeout(() => {
      console.log("Dashboard1 initial load");
      setIsLoaded(true);
      // Force data refresh
      setPeriod(prev => prev);
    }, 100);
    ...
    const refreshTimer = setTimeout(() => {
      console.log("Dashboard1 forcing refresh");
      setPeriod(prev => prev);
    }, 300);
    ...
    return () => {
      ...
      console.log("Dashboard1 unmounted");
    };
  }, []);
  ...
  console.log("Dashboard1 rendering with period:", period, "isLoaded:", isLoaded);
      ...
      <ResponsiveContainer className="pt-6 pb-10">
        <div className="flex flex-col space-y-6">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
              <h1 className="text-2xl md:text-3xl font-bold tracking-tight">Dashboard 1</h1>
